...
class TournesolAPI:
	...
	def call_options(self, path: str, body: dict[str,any]=None):
		self._call(requests.options, path, body)
	...
	def setVideoPublic(self, vid:str, isPublic=True, saveCache=True) -> VData:
		updated_vdata = self.call_patch(f"users/me/contributor_ratings/videos/{vid}/", {"is_public":isPublic})
		self._cache_vdata(updated_vdata, timestamp())
	...
	def getVData(self, vid: str, useCache=False, saveCache=True) -> VData:
		...
		if not useCache or vid not in self.cache['all/videos']:
			...
			if not got:
				...
				resp = self.call_post(f"/video/", body={'video_id': vid[3:]})
				if 'uid' in resp:
					got = self.call_get(f"/polls/videos/entities/{vid}")
				else:
