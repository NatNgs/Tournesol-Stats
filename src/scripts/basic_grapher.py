...
def draw_graph_to_file(graph: nx.Graph, videos: dict[str, Video], filename: str):
	...
	for edge in graph.edges.data():
		if edge[2]['cmp_by_me']:
			...
			edge[2]['weight'] = edge[2]['cmps'] if edge[2]['cmps'] > 0 else 0.5
	subgraph: nx.Graph = graph.subgraph(vids_to_show)

	pos = nx.spring_layout(subgraph, pos=nx.circular_layout(subgraph), weight='weight')
	...
	# Lang color
	langs = list({videos[node].channel.lang for node in subgraph.nodes if node in videos})
	if '??' in langs:
		langs.remove('??')
	nblangs = len(langs)
